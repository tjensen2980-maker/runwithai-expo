...
class RailwayManager: NSObject, ObservableObject {
        ...
        URLSession.shared.dataTask(with: request) { [weak self] responseData, response, error in
            ...
            DispatchQueue.main.async {
                if success {
                    ...
                    print("[Railway] Upload fejlede: \(statusCode), \(error?.localizedDescription ?? "unknown")")
                    self?.savePendingRun(workoutData)
                    completion?(false)
                }
            }
    ...
    func fetchTodayTraining(completion: @escaping ([String: Any]?) -> Void) {
        guard let token = jwtToken else { completion(nil); return }
        guard let url = URL(string: "\(baseURL)/weekplan") else { completion(nil); return }
