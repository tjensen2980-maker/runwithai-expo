...
class SyncManager: ObservableObject {
    ...
    private func syncNext(workouts: [Workout], index: Int, successes: Int) {
        ...
        postWorkout(workout) { [weak self] success in
            ...
            if success {
                ...
                DispatchQueue.main.async {
                    self.isSyncing = false
                    self.lastSyncStatus = "Fejl v. \(index+1). Proever igen"
                }
            }
        }
    }
    ...
    private func postWorkout(_ workout: Workout, completion: @escaping (Bool) -> Void) {
        ...
        let hrArray: [[String: Any]] = workout.hrSamples.map { sample in
            return [
                "t": isoFmt.string(from: sample.t),
                "bpm": sample.bpm
            ]
        }
        var hrSamplesString = "[]"
        if let hrData = try? JSONSerialization.data(withJSONObject: hrArray),
           let str = String(data: hrData, encoding: .utf8) {
            hrSamplesString = str
        }

        var splitsArray: [[String: Any]] = []
        if workout.route.count > 1 {
            var lastSplitDist: Double = 0
            var lastSplitTime: Date = workout.route[0].timestamp
            var cumDist: Double = 0
            for i in 1..<workout.route.count {
                let p1 = workout.route[i-1]
                let p2 = workout.route[i]
                let dLat = (p2.lat - p1.lat) * 111000.0
                let dLon = (p2.lon - p1.lon) * 111000.0 * cos(p1.lat * .pi / 180.0)
                cumDist += sqrt(dLat*dLat + dLon*dLon)
                if cumDist - lastSplitDist >= 1000.0 {
                    let splitTime = p2.timestamp.timeIntervalSince(lastSplitTime)
                    splitsArray.append([
                        "km": splitsArray.count + 1,
                        "duration": Int(splitTime),
                        "pace": splitTime / 60.0
                    ])
                    lastSplitDist = cumDist
                    lastSplitTime = p2.timestamp
                }
            }
        }
        var splitsString = "[]"
        if let splitsData = try? JSONSerialization.data(withJSONObject: splitsArray),
           let str = String(data: splitsData, encoding: .utf8) {
            splitsString = str
        }
        ...
        let runBody: [String: Any] = [
            ...
            "total_ascent": workout.totalAscent,
            "total_descent": workout.totalDescent,
    ...
    private func todayShortDa() -> String {
        let days = ["Soen", "Man", "Tir", "Ons", "Tor", "Fre", "Loer"]
    ...
    private func saveTrainingPlan(_ planData: [[String: Any]]) {
        ...
        session.dataTask(with: request) { _, response, _ in
            ...
            DispatchQueue.main.async {
                if let t = TrainingManager.shared.todayTraining {
