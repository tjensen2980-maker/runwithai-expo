...
if [ -s "$NVM_DIR/nvm.sh" ]; then
  . "$NVM_DIR/nvm.sh"; nvm install 20; nvm use 20; NPM_BIN="$(which npm)"
else
...
if command -v pod >/dev/null 2>&1; then
  ...
  brew install cocoapods || true; POD_BIN="$(brew --prefix)/bin/pod"
fi
echo "Pod: $POD_BIN"
...
sed -i '' 's/CURRENT_PROJECT_VERSION = [^;]*;/CURRENT_PROJECT_VERSION = 202;/g' "$PBXPROJ"
...
# Expo SDK 54+ handles entry file automatically via package.json "main"
"$NPM_BIN" exec --yes -- expo export:embed \
  ...
  echo "expo export:embed failed, trying react-native bundle with AppEntry.js"
...
for f in WorkoutManager.swift WatchConnectivityManager.swift ContentView.swift RunningView.swift WorkoutSummaryView.swift TrainingPickerView.swift TrainingPlan.swift RunWithAI_WatchApp.swift RunUploader.swift; do
  if [ -f "$WATCH_SRC/$f" ]; then cp "$WATCH_SRC/$f" "$WATCH_DST/$f"; fi
done
