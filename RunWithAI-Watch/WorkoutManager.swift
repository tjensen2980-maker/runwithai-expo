...
// MARK: - Pace Status (Garmin-style target pace feedback)
enum PaceStatus {
...
class WorkoutManager: NSObject, ObservableObject {
              ...
              // MARK: - Target pace (Garmin-style)
              @Published var targetPaceMin: Double = 0  // seconds/km lower bound (faster)
              @Published var targetPaceMax: Double = 0  // seconds/km upper bound (slower)
              ...
              // MARK: - Target Pace (set from training plan)
              func setTargetPace(minPace: Double, maxPace: Double, label: String = "") {
                                ...
                                paceStatus = (minPace > 0 || maxPace > 0) ? .noTarget : .noTarget
              }

              private func updatePaceStatus() {
                                guard targetPaceMax > 0, smoothedPace > 0 else {
                                                      paceStatus = .noTarget
                                                      return
                                }
                                let tolerance = 10.0 // 10 seconds tolerance
                                if smoothedPace < targetPaceMin - tolerance {
                                                      paceStatus = .tooFast
                                } else if smoothedPace > targetPaceMax + tolerance {
                                                      paceStatus = .tooSlow
                                } else {
                                                      paceStatus = .onTarget
                                }
              ...
              private func resetWorkoutData() {
                                ...
                                showSummary = false
                                paceStatus = targetPaceMax > 0 ? .noTarget : .noTarget
              }
              ...
              private func stopStepCounting() {
                                if let query = stepQuery { healthStore.stop(query) }
              }
              ...
              func checkKmSplit() {
                                ...
                                if currentKm > previousKm && currentKm > 0 {
                                                      ...
                                                      let avgHR = currentKmHRSamples.isEmpty
                                                          ? heartRate
                                                          : currentKmHRSamples.reduce(0, +) / Double(currentKmHRSamples.count)
              ...
              private func recordPartialSplit() {
                                let splitDistance = distance - currentKmStartDistance
                                guard splitDistance > 50 else { return }
                                let splitTime = elapsedSeconds - currentKmStartTime
                                let splitPace = splitDistance > 0 ? (Double(splitTime) / splitDistance) * 1000 : 0
                                let avgHR = currentKmHRSamples.isEmpty
                                    ? heartRate
              ...
              private func updateSmoothedPace(_ rawPace: Double) {
                                ...
                                guard rawPace > 120 && rawPace < 1200 else { return }
              ...
              func formatDistance(_ meters: Double) -> String { String(format: "%.2f", meters / 1000) }
              ...
              func formatElevation(_ meters: Double) -> String { String(format: "%.0f m", meters) }

              // MARK: - Heart Rate Zone
              func heartRateZone() -> Int {
                                let pct = heartRate / 190.0
                                switch pct {
                                                  case ..<0.6:    return 1
              ...
              func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
                                ...
                                if let latest = filteredLocations.last {
                                                      ...
                                                      if let prev = previousAltitude {
                                                                                ...
                                                                                DispatchQueue.main.async {
                                                                                                              if diff > 0 { self.totalAscent += diff }
                                                                                                              else { self.totalDescent += abs(diff) }
