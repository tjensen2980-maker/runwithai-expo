...
class WatchConnectivityManager: NSObject, ObservableObject {
            ...
            // MARK: - Today's Training Plan (modtaget fra iPhone)
...
extension WatchConnectivityManager: WCSessionDelegate {
            func session(_ session: WCSession, activationDidCompleteWith activationState: WCSessionActivationState, error: Error?) {
                            DispatchQueue.main.async {
                                                ...
                                                // Request today's training as soon as session is active
            ...
            func sessionReachabilityDidChange(_ session: WCSession) {
                            DispatchQueue.main.async {
                                                self.isReachable = session.isReachable
                                                // Request today's training when phone becomes reachable
            ...
            func session(_ session: WCSession, didReceiveMessage message: [String: Any]) {
                            DispatchQueue.main.async {
                                                ...
                                                // Handle today's training plan pushed from iPhone
            ...
            // Handle UserInfo transfers (for background delivery of training plan)
            func session(_ session: WCSession, didReceiveUserInfo userInfo: [String: Any] = [:]) {
                            DispatchQueue.main.async {
                                                if let type = userInfo["type"] as? String, type == "TODAY_TRAINING" {
                                                                        self.todayTraining = userInfo
                                                }
                            }
            }
}
