...
const IconPlan = ({ active }) => {
  const color = active ? colors.accent2 : colors.muted;
  return (
...
// Watch sync - modtager run fra ur og marker dagens træning som completed
  ...
  // HealthKit auto-sync til Railway fjernet - vi læser direkte i
...
if (tab === 'cycleTracker') {
  ...
  const renderScreen = () => {
    ...
    case 'dashboard':
      ...
      case 'activity':
        return <Progress runs={runs} isPro={isPro} onRunDeleted={(deletedRun) => {
