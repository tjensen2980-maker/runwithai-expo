...
export function useHealthKit({ enabled = true, heartRateInterval = 5000 } = {}) {
        ...
        if (Platform.OS !== 'ios' || !enabled) {
                ...
                const initHealthKit = () => {
                        if (!AppleHealthKit || typeof AppleHealthKit.isAvailable !== 'function') {
                                  Alert.alert('HK INIT', 'AppleHealthKit module is NULL');
                                  setIsInitializing(false);
                                  return;
                        }
  ...
  /**
     ...
     * Bruges til ernærings-dashboard (ikke løb-tracking).
     */
  const fetchDailyCalories = useCallback(() => {
        ...
        startOfDay.setHours(0, 0, 0, 0);
                                             ...
                                             AppleHealthKit.getActiveEnergyBurned(options, (err, results) => {
                                                     if (err) {
                                                               console.warn('[HealthKit] Daily calories fetch error:', err);
                                                               return;
                                                     }
                                                     if (results && results.length > 0) {
                                                               const total = results.reduce((sum, s) => sum + s.value, 0);
                                                               setCalories(Math.round(total));
                                                               console.log('[HealthKit] Daily active calories:', Math.round(total));
                                                     } else {
                                                               setCalories(0);
                                                               console.log('[HealthKit] Daily active calories: 0 (ingen data)');
                                                     }
                                             });
  }, [isAuthorized]);
